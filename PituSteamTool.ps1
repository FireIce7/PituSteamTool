...
if ($lang -eq "pt") {
    ...
    $S.AdminFail        = "[!] Не удалось получить права администратора."
    $S.AdminTip1        = "Щёлкните правой кнопкой мыши по PowerShell и"
    $S.AdminTip2        = "выберите 'Запустить от имени администратора'."
    $S.PressEnter       = "Нажмите ENTER для выхода"
    $S.SteamInfo        = "Steam Info"
    $S.Folder           = "[Папка]  "
    $S.FolderNotFound   = "!  Папка не найдена"
    $S.Steam            = "[Steam]  "
    $S.SteamExeNF       = "!  Steam.exe не найден"
    $S.CfgLabel         = "[Config] "
    $S.CfgActive        = "Downgrade активен (steam.cfg присутствует)"
    $S.CfgDefault       = "Стандартно (без блокировки версии)"
    $S.MainMenu         = "Главное меню"
    $S.Opt1             = ">> Downgrade до Steam 32-bit"
    $S.Opt2             = "<< Upgrade / Steam 64-bit"
    $S.Opt3             = "** Глубокое восстановление (сохранит игры)"
    $S.OptExit          = "Выход"
    $S.Choose           = "Выбор"
    $S.Bye              = "До встречи!"
    $S.InvalidOpt       = "Неверный вариант. Попробуйте снова."
    $S.Step             = "ШАГ"
    $S.Downloading      = "Загрузка"
    $S.DlFail           = "Ошибка загрузки:"
    $S.ZipEmpty         = "Пустой ZIP"
    $S.ZipInvalid       = "Файл не является ZIP"
    $S.Extracting       = "Распаковка:"
    $S.Unknown          = "Неизвестно"
    $S.ClosingSteam     = "Завершение процессов Steam"
    $S.NoSteamProc      = "Нет процессов Steam"
    $S.AllClosed        = "Все процессы завершены"
    $S.Attempt          = "Попытка"
    $S.Of               = "из"
    $S.Killed           = "Завершён:"
    $S.SvcStopped       = "Служба Steam остановлена"
    $S.SvcNeedAdmin     = "SteamService требует прав администратора"
    $S.CantKill         = "Не удалось завершить:"
    $S.StillRunning     = "Некоторые процессы ещё работают"
    $S.SteamClosed      = "Steam успешно закрыта"
    $S.PathNotFound     = "Папка Steam не найдена:"
    $S.EditTip          = "Если Steam в другой папке, измените скрипт:"
    $S.ExeNotFound      = "Steam.exe не найден"
    $S.DgTitle          = "DOWNGRADE"
    $S.DgSub            = "Steam 32-bit"
    $S.DgIntro          = "Этот процесс выполнит:"
    $S.DgS1             = "1. Закроет Steam"
    $S.DgS2             = "2. Скачает 32-bit файлы с GitHub"
    $S.DgS3             = "3. Заменит файлы в папке Steam"
    $S.DgS4             = "4. Создаст steam.cfg для блокировки обновлений"
    $S.DgS5             = "5. Откроет Steam с -clearbeta"
    $S.LocalOnly        = "100% локально - без удалённых скриптов."
    $S.TypeYes          = "ДА"
    $S.TypeConfirm      = "для подтверждения: "
    $S.Cancelled        = "отменено пользователем."
    $S.DlBinFail        = "Не удалось скачать файлы."
    $S.DlSteam32        = "Загрузка Steam 32-bit"
    $S.ExtractOver      = "Распаковка в папку Steam"
    $S.BinsInstalled    = "32-bit файлы установлены!"
    $S.ExtractErr       = "Ошибка распаковки:"
    $S.UpdMill          = "Обновление Millennium (DLL обнаружены)"
    $S.Found            = "Найдено:"
    $S.MillUpdated      = "Millennium обновлён!"
    $S.MillExtErr       = "Ошибка распаковки Millennium:"
    $S.MillDlFail       = "Millennium не обновлён (загрузка не удалась)"
    $S.CreatingCfg      = "Создание steam.cfg (блокировка обновлений)"
    $S.CfgCreatedRO     = "steam.cfg создан и защищён (только чтение)"
    $S.CfgCreated       = "steam.cfg создан"
    $S.VerifyOpen       = "Проверка и запуск Steam"
    $S.Confirmed32      = "Steam.exe подтверждён как 32-bit!"
    $S.Still64          = "Steam.exe всё ещё 64-bit. Попробуйте Глубокое восстановление."
    $S.CantVerify       = "Не удалось проверить архитектуру"
    $S.OpenedClear      = "Steam открыта с -clearbeta"
    $S.DgDone           = "DOWNGRADE ЗАВЕРШЁН"
    $S.DgMsg1           = "Ваши игры и данные в безопасности."
    $S.DgMsg2           = "Steam откроется в 32-bit."
    $S.DgWarn1          = "НЕ обновляйте Steam иначе"
    $S.DgWarn2          = "downgrade будет отменён!"
    $S.UpTitle          = "UPGRADE"
    $S.UpSub            = "Steam 64-bit"
    $S.UpIntro1         = "Этот процесс снимает блокировку downgrade"
    $S.UpIntro2         = "и очищает кеш для обновления."
    $S.UpWhatHappens    = "Что произойдёт:"
    $S.UpS1             = "1. Закроет Steam"
    $S.UpS2             = "2. Переименует steam.cfg (блокировку)"
    $S.UpS3             = "3. Очистит кеш пакетов"
    $S.UpS4             = "4. Откроет Steam (обновится сама)"
    $S.UpSafe           = "Ваши игры и данные не будут затронуты."
    $S.CloseSteam       = "Закрытие Steam"
    $S.SteamDone        = "Steam закрыта"
    $S.RemoveLocks      = "Снятие блокировки downgrade"
    $S.CfgRenamed       = "steam.cfg -> steam.cfg.backup"
    $S.CfgNotFound      = "steam.cfg не найден (OK)"
    $S.BetaRemoved      = "package\beta удалён"
    $S.BetaNotFound     = "package\beta не найден (OK)"
    $S.ClearCache       = "Очистка кеша пакетов"
    $S.FilesRemoved     = "файл(ов) удалено из кеша"
    $S.CacheClean       = "Кеш уже был чистым"
    $S.OpenSteam        = "Запуск Steam"
    $S.SteamStarted     = "Steam запущена"
    $S.UpDone           = "UPGRADE ЗАПУЩЕН"
    $S.UpNow            = "ЧТО ВАМ НУЖНО СДЕЛАТЬ СЕЙЧАС:"
    $S.UpTip1           = "1. Steam скачает обновление"
    $S.UpTip2           = "   64-bit автоматически. Ждите."
    $S.UpTip3           = "2. Проверьте: Диспетчер задач"
    $S.UpTip4           = "   (Ctrl+Shift+Esc) > Steam Client"
    $S.UpTip5           = "   Если НЕТ '(32-bit)' — всё OK!"
    $S.UpFallback1      = "Если не работает, используйте опцию 3"
    $S.UpFallback2      = "(Глубокое восстановление)."
    $S.RpTitle          = "ГЛУБОКОЕ ВОССТАНОВЛЕНИЕ"
    $S.RpIntro1         = "Этот режим удаляет ВСЁ из папки Steam,"
    $S.RpIntro2         = "кроме элементов ниже которые "
    $S.RpPreserved      = "сохранены"
    $S.RpExe            = "Steam.exe    (исполняемый файл)"
    $S.RpApps           = "steamapps    (установленные игры)"
    $S.RpUser           = "userdata     (сохранения и настройки)"
    $S.RpCfg            = "config       (аккаунтные данные)"
    $S.RpElse1          = "Всё остальное (DLL, кеш, скины, package)"
    $S.RpElse2          = "будет удалено. Steam заново скачает"
    $S.RpElse3          = "то что нужно при открытии."
    $S.RpWarn           = "Необратимая операция! Сделайте бекап."
    $S.RpCancelled      = "Восстановление отменено пользователем."
    $S.RpRemoving       = "Удаление старых файлов"
    $S.RpRemoved        = "элемент(ов) удалено из"
    $S.RpRebuilding     = "Запуск Steam для восстановления"
    $S.RpSpinner        = "Запуск Steam..."
    $S.RpDone           = "ВОССТАНОВЛЕНИЕ ЗАВЕРШЕНО"
    $S.RpNow            = "ЧТО ВАМ НУЖНО СДЕЛАТЬ СЕЙЧАС:"
    $S.RpTip1           = "1. Steam откроется и скачает"
    $S.RpTip2           = "   все файлы с нуля."
    $S.RpTip3           = "   Это может занять несколько минут."
    $S.RpTip4           = "2. Войдите в аккаунт."
    $S.RpTip5           = "   Ваши игры будут в библиотеке"
    $S.RpTip6           = "   готовы к игре."
    $S.RpSafe           = "Ваши игры и сохранения в безопасности!"
    $S.PressEnterCont   = "для продолжения..."
    $S.Type             = "Введите "
} else {
